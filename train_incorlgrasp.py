...
# beta vae
parser.add_argument('--ex_latent_size', dest='ex_latent_size', type=int, default=256, help='')
parser.add_argument('--in_latent_size', dest='in_latent_size', type=int, default=256, help='')
...
parser.add_argument('--image_height', dest='image_height', type=int, default=360, help='')
parser.add_argument('--image_width', dest='image_width', type=int, default=360, help='')
...
parser.add_argument('--obj-detector-dir', dest='obj_detector_dir', type=str, default='./obj_detector/checkpoints')
parser.add_argument('--arm-segmentation-dir', dest='arm_segmentation_dir', type=str, default='./arm_segmentation/checkpoints')
...
#    q_summary = tf.summary.scalar(name='q', tensor=tensorboard_q)
    ...
    print('actor_var')
    actorfactory = ActorFactory(args.nb_actors, memory_factory.onlineMemory, -1, None, args.offline_data_path, args, nb_grasp, None)
    print('ActorFactory')
    sleep(10)
...
#            print('nb_grasp:', nb_grasp)
            for t_train in range(args.nb_train_steps):
                grasp_ratio = (args.max_grasp_nb - nb_grasp) / args.max_grasp_nb
                offlind_data_ratio = 0.5 * grasp_ratio
                if offlind_data_ratio < 0.5:
                    offlind_data_ratio = 0.5

                training_data = memory_factory.getdata(args.ddpg_batch_size)
        ...
        if training_step % (args.nb_train_steps * 25) == 0 and training_step > 0:
            ...
            mean_eval_returns = actorfactory.get_mean_eval_returns()
            summary_reward = np.mean(mean_eval_returns)

#            mean_eval_qs = actorfactory.get_mean_eval_qs()
#            summary_q = np.mean(mean_eval_qs)

            nb_grap_success = actorfactory.get_mean_grasp_success()
            mean_nb_grap_success = np.mean(nb_grap_success)
            std_nb_grap_success = np.std(nb_grap_success)


            feed_dict = {tensorboard_loss: summary_loss, tensorboard_reward: summary_reward,
                         tensorboard_nb_success: mean_nb_grap_success, tensorboard_std_success: std_nb_grap_success}
